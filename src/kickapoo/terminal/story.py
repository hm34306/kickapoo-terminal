...
class TerminalStoryTeller():
    ...
    def __init__(self, writing_speed: int = 100, theme: str=Themes.DEFAULT):
        self._writing_speed = writing_speed
        set_theme(theme)

    def _write(self, message: str, font_color:Colorful.ColorfulStyle, line_max:int):
        if font_color:
            print(font_color)
    ...
    def paragraph(self, message: str, font_color:Colorful.ColorfulStyle=None, line_max: int=88)-> None:
        self._write(message, font_color, line_max=line_max)
        print("\n")

    def list_writer(self, items: List[str], symbol: ListSymbol=ListSymbol.BULLETS, font_color:Colorful.ColorfulStyle=None, line_max: int=88) -> None:
        current_symbol_value = symbol.value
        symbol_suffix = "" if symbol == ListSymbol.BULLETS else "."
        for item in items:
            self._write(f"{current_symbol_value}{symbol_suffix} {item}\n", font_color, line_max)
    ...
    def code_format(self, code: str, code_type: code_formatter.CodeType = code_formatter.CodeType.PYTHON) -> None:
...
reveals a unique and dynamic atmosphere."""
