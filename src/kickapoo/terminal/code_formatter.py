...
def format_code(code, code_type=CodeType.PYTHON):
    """Format and print code with syntax highlighting using colorful"""
    print(f"{cf.yellow}<--- BEGIN {Symbols.PYTHON} {cf.magenta}<{code_type}>{cf.yellow} Code Block --->{FontFormat.RESET}")
            ...
            while i < len(line_without_indent):
                ...
                if i < len(line_without_indent) and line_without_indent[i] in ["'", '"']:
                    ...
                    while j < len(line_without_indent):
                        if line_without_indent[j] == quote and line_without_indent[j-1] != '\\':
                            break
                        j += 1
                ...
                # Check for keywords
                keywords = ['def', 'class', 'import', 'from', 'for', 'in', 'if', 'else', 'elif', 
                           'while', 'return', 'True', 'False', 'None', 'and', 'or', 'not', 
                           'try', 'except', 'finally', 'with', 'as', 'break', 'continue', 
                           'pass', 'raise', 'assert', 'is', 'lambda', 'yield']
                
                # Extract a word if we're at the beginning of one
                if i < len(line_without_indent) and (line_without_indent[i].isalpha() or line_without_indent[i] == '_'):
                    j = i + 1
                    while j < len(line_without_indent) and (line_without_indent[j].isalnum() or line_without_indent[j] == '_'):
                        j += 1
                ...
                if i < len(line_without_indent) and line_without_indent[i].isdigit():
                    j = i + 1
                    while j < len(line_without_indent) and (line_without_indent[j].isdigit() or line_without_indent[j] == '.'):
                        j += 1
